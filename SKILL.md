...
**Run mode (resolve FIRST — it decides whether the whole-site residue gate applies).** Before content work, resolve the run mode with `scripts/resolve_run_mode.py --site-creation-status <created_verified|existing_site_selected>`. A newly created site is `from_scratch` — it always ships AllinCMS default-template demo content, so `check_template_residue` is required (no prompt). A selected EXISTING site is ambiguous: it may be a `template_conversion` (old brand/products/demo scattered across it → residue gate required) or an `incremental_update` (a site that is already yours and clean → residue gate skipped, only new/changed items content-quality checked). The skill CANNOT tell these apart, so it MUST ASK the user, and until answered it defaults to conversion (residue gate ON). `incremental_update` is never a silent default — only an explicit user answer selects it, biasing every ambiguous case toward scanning rather than silently shipping leftover template content. The content-quality gate applies in every mode.
...
- `references/workspace-layout.md`: read when there are MULTIPLE clients and/or a client has MULTIPLE sites. Defines the PRIVATE multi-client/multi-site workspace kept OUTSIDE this public skill (client materials/contacts/real siteKeys are PII that must never enter the public repo): one client per folder with append-only `raw/` + distilled client-level `wiki/` (company/brand/contact shared across that client's sites), each site an isolated subfolder (`source-wiki`/`package`/`run`/`residue-blacklist`/`live`). Client-level shared, site-level isolated. Scaffold with `scripts/init_workspace.py`.
- `references/build-pipeline.md`: read to run the end-to-end build for one workspace site. Stitches the source→package→build chain to the three gates: distill client wiki → build the site's `source-wiki` → `package` → **content-quality gate** → **resolve run-mode** → authorized browser build → **residue gate** (from_scratch/conversion only) → launch acceptance, each artifact landing in the site's `run/`. Use `scripts/site_build_status.py --site-dir <site>` to see what's done, the next step, and which gate is still owed — and to resume a client's multiple sites without losing the thread.
...
- Full helper-script index (one line each) lives in `references/script-index.md` — consult it for "which script does what"; each script also has `--help`. The Workflow steps below invoke the ones you run by hand with full commands. Gates you must not skip: `check_pre_mutation_gate.py` (before every remote mutation); `validate_manifest.py --require-schema-verified` + `validate_slate_content_shape.py` + `check_content_quality.py --package <package>` (pre-publish content quality — stock/placeholder images, missing alt, unreplaced template copy = block; off-category specs / thin spec table / duplicate copy = warn + AI self-review) + `check_content_format.py --package <package>` (readability/structure — a long body with zero headings/lists/bold = paragraph-WALL block; too few section headings / no bold / no list / no CTA / over-long paragraphs = warn per `content-format-standard.md`; use `--strict` on product/post bodies so structural warnings block too. Author generation-first: run `build_content_skeleton.py --package <package> --apply` FIRST so each body is correct-by-construction, then fill the `[fill: ...]` leaf text — the gate is a safety net, not the author; it guarantees structure, never prose quality/truth, which stays an AI self-review + human-review job) + `check_next_action_freshness.py` (before a content batch); `validate_theme_page_document.py` (theme JSON replay body); `check_template_residue.py --blacklist <old-fingerprints> --frontend <every-live-route-text>` (applies when run mode is from_scratch/template_conversion per `resolve_run_mode.py`, skipped only for a user-confirmed incremental_update — whole-site scan for old brand/product/category/tag/contact residue; any hit blocks launch until zero); `audit_skill_hygiene.py` + `check_round_closeout.py` (turn closeout).
