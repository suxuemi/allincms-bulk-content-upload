...
- Full helper-script index (one line each) lives in `references/script-index.md` — consult it for "which script does what"; each script also has `--help`. The Workflow steps below invoke the ones you run by hand with full commands. Gates you must not skip: `check_pre_mutation_gate.py` (before every remote mutation); `validate_manifest.py --require-schema-verified` + `validate_slate_content_shape.py` + `check_content_quality.py --package <package>` (pre-publish content quality — stock/placeholder images, missing alt, unreplaced template copy = block; off-category specs / thin spec table / duplicate copy = warn + AI self-review) + `check_next_action_freshness.py` (before a content batch); `validate_theme_page_document.py` (theme JSON replay body); `check_template_residue.py --blacklist <old-fingerprints> --frontend <every-live-route-text>` (applies when run mode is from_scratch/template_conversion per `resolve_run_mode.py`, skipped only for a user-confirmed incremental_update — whole-site scan for old brand/product/category/tag/contact residue; any hit blocks launch until zero); `audit_skill_hygiene.py` + `check_round_closeout.py` (turn closeout).
